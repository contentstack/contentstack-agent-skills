...
description: "Wire a CATCH-ALL Studio route so every visitor URL resolves through Studio via `<StudioComponent />`. Direct visits and in-Studio preview share one mount. Includes explicit opt-outs."
allowed-tools: Read Grep Glob
---

## When to use

Wire a CATCH-ALL Studio route so every visitor URL resolves through Studio via `<StudioComponent />`. Direct visits and in-Studio preview share one mount. Includes explicit opt-outs.

Use when adding Studio-rendered template routes (Vite/React Router or Next.js App Router), seeing "Template did not load" / "COMPOSITION_NOT_FOUND" on a template URL, or finishing the Studio setup chapter (after `install-studio` + `setup-section-preview`). Phrases — "make every URL go through Studio", "wire catch-all template route", "template URL 404s". Skip if only the section-authoring `/canvas` route is needed (that's `setup-section-preview`).

# Wire template preview routes

## Context

The user already has Studio installed (`install-studio`) and a `/canvas` section-authoring route mounted (`setup-section-preview`). They now need to wire Studio as the **default renderer for every URL on the site** — so any published composition resolves to its template automatically, and any opened template in Studio's iframe renders live.

**Catch-all is the recommended default, not mandatory.** This skill registers ONE route matching every URL. Studio resolves the composition by URL at render time via `sdk.fetchCompositionData({ url, searchQuery })`; no match → null → catch-all renders 404. Per-template route registration is **not needed** — Studio's URL patterns handle matching internally — but it's valid if you have a reason (see below).
...
1. **Confirm prerequisites.** Bail with a clear message if any of these are missing:
   ...
   - A `/canvas` route mounting `<StudioCanvas />` already exists (created by `setup-section-preview`).
...
2. **Confirm the catch-all default + collect opt-outs.** Tell the user verbatim:
   ...
   If the user provides opt-out paths, register them ahead of the catch-all (step 4). If they leave empty, only the catch-all is wired.

3. **Create one shared route component.** Do NOT generate a separate page per template — every template route uses identical wiring. One generic component handles all of them.
   ...
   **React Router (Vite/CRA) variant** — emit at the path the user chose (default `src/routes/LinkedRoute.tsx`):
   ...
   export function LinkedRoute() {
     ...
     if (error) return <div>No composition for {pathname}</div>;
   ...
   **App Router CSR variant (SPAs only — no SEO requirement)** — emit at `app/_components/LinkedRoute.tsx`, marked `"use client"`. Skip this for any visitor-facing site; the SSR pattern above is the default:
   ...
   - **`searchQuery` = raw URLSearchParams object.** When Studio's iframe loads the template URL, it appends editor-mode params. The SDK reads them off `searchQuery` to switch from "render deployed composition" to "render unsaved working spec." Omit `searchQuery` and the SDK falls through to the deployed path → returns `null` on a not-yet-saved composition → **"Template Did Not Load."**
   - **Return an empty `<div />` (not `null`) when there's no spec.** Studio's canvas host queries the mount target after route render; if there's nothing in the DOM it can't drive it, and Studio shows "SDK Not Initialized" or a blank error. An empty div gives it something to attach to.
...
4. **Register the catch-all route — pick `[...slug]` vs `[[...slug]]` based on existing app pages.**
   ...
   ```bash
   # Does the app already have a root page.tsx?
   ls app/page.tsx pages/index.tsx 2>/dev/null

   # What other specific routes exist? (informational — they win by Next precedence)
   find app/ pages/ -name 'page.tsx' -o -name 'page.jsx' 2>/dev/null | head -20
   ```
   ...
   **Branching rule:**
   ...
   | No root page found → greenfield-style | `app/[[...slug]]/page.tsx` — **optional** catch-all. Matches `/` AND all nested URLs. |
   ...
   **React Router** — in `src/main.tsx` (or wherever the route table lives), register opt-outs FIRST (so they take precedence) and the catch-all LAST:
   ...
   **Next.js App Router** — based on the detection above, create ONE catch-all file. Left = existing root page (use `[...slug]`); right = greenfield (use `[[...slug]]`):
   ...
   ![Next.js App Router catch-all layouts — existing root page uses [...slug]; greenfield uses [[...slug]]](../../docs/assets/svg/template-route-layouts-nextjs.svg)

   The catch-all itself must call `notFound()` when `!specOptions?.spec` so unclaimed paths 404 cleanly:

   ```tsx
   // app/[...slug]/page.tsx OR app/[[...slug]]/page.tsx
   import { notFound } from "next/navigation";
   // …after fetchCompositionData…
   if (!specOptions?.spec) notFound();
   ```
...
6. **Smoke-test both code paths.**
   ...
   3. Open Studio → open a template → Preview Entry → pick a real entry. Studio loads the visitor URL inside its iframe. The canvas should render the template bound to that entry's data — `<StudioComponent />` detects iframe context internally and shows the live in-canvas runtime.
...
1. `optOutPaths` — comma-separated list of route patterns that should NOT go through Studio. **Leave empty (default) to keep the catch-all unrestricted — RECOMMENDED for most apps.** Provide opt-outs only if the user has explicit non-Studio routes (`/api/*`, `/admin/*`, etc.).
2. `sharedRoutePath` — where the shared `LinkedRoute` component should live. Default `src/routes/LinkedRoute.tsx` for Vite, `app/_components/LinkedRoute.tsx` for Next.js App Router.
...
- [ ] **ONE catch-all route** is registered. For Next.js: `app/[...slug]/page.tsx` (non-optional) WHEN the app has an existing `app/page.tsx`, OR `app/[[...slug]]/page.tsx` (optional) WHEN greenfield with no root page. For React Router: `<Route path="*">` registered LAST. The catch-all points at `LinkedRoute`.
...
**Recommended:** audit the app's global `<Script>`s / analytics tags on the routes Studio renders. Third-party loaders that `throw` on failure should be either (a) removed from Studio's routes via a route-group / layout guard, or (b) patched to `console.warn` + `return` instead of `throw`. See [`troubleshoot-canvas`](../troubleshoot-canvas/SKILL.md) § *Third-party global-script overlays* for the full playbook.
