...
description: "End-to-end orchestrator for the BYOC arc — chains register-component (lazy) → import-design-tokens → build-section → use-section-slot → build-connected-template → configure-csr-vs-ssr → setup-template-preview-routes → verify-setup."
...
Gate: open `<env Base URL>` + `<Template URL>` directly in a browser (not through Studio). The Template renders with real data. Open the same Template in Studio's Templates tab → the iframe shows the same render, with edit overlays.
...
| Forgetting Phase 6 ("verify + ship") | Template renders on the canvas → user assumes prod will be fine → CSP headers blocking the iframe in prod, env Base URL pointing at localhost in prod, etc. | `verify-setup` and `deploy-studio-site` are mandatory phases, not optional. The plan doesn't end at "renders on canvas." | <!-- style-lint: allow -->
