---
name: figma-generate-components
description: "Generate React components from a Figma design and register them with Studio via the Studio Figma Plugin + `csdx studio:component:sync/add/register` CLI commands. Optionally imports Figma design tokens."
...
- `/docs/20-bring-your-own-components/generate-components-from-figma.md` — the full developer-facing flow with reference diagram, four CLI commands, plugin walkthrough, screenshots
- `/docs/20-bring-your-own-components/copy-and-paste-from-figma-to-studio.md` — the lighter-weight Copy to Studio path
- `/docs/20-bring-your-own-components/configure-design-tokens-in-studio.md` — the design-token side
...
- **`Sync`** flows existing code → plugin (one-way, upstream).
- **`Add`** flows plugin's Figma output → new code in the repo (one-way, downstream).
...
If the user **doesn't have a Component ID yet**, walk them through the full setup first (Steps 1–3), then have them open Figma and run the plugin (Step 3), then come back for Step 4.
...
csdx config:set:region   # pick the user's Contentstack region
csdx login --oauth        # opens browser for OAuth
```
...
>
> 1. Install **Contentstack Studio** from the Figma Marketplace (plugin id 1541766192464484605) if not already installed.
> 2. Open the file → Plugins → Contentstack Studio → click **Run**.
> 3. Pick region, click **Authorize** — completes OAuth, returns to Figma.
> 4. Confirm the Figma file is **editable** (read-only files block Auto Layout updates and mapping).
> 5. Select the frame(s) or component(s) to generate (Shift+click for multiple).
> 6. Add a **layout prompt** if needed (e.g. *"Make this a carousel"*). Prompts shape layout, spacing, and responsiveness only — not behavior.
> 7. Pick a **responsiveness** option: *Optimize for responsiveness* (desktop/tablet/mobile) or *Fixed-size screens*.
> 8. Let **Auto Map** match Figma layers to your synced code components — override any row manually.
> 9. Click **Generate Design**.
> 10. The plugin shows four output buttons. Click **Copy CLI Command** to get a ready-to-run `csdx studio:component:add --component-id=<id>` on your clipboard. If design tokens were generated, also note the Design Token ID.
...
If the user wants to customise the generation, pass an additional prompt argument to the `Add` command (refer to the CLI's `--help` for the exact flag — varies by CLI version). Common customisations:
...
1. **Include Studio's default design tokens?** (Y/N) — default Y to keep canonical Studio tokens alongside custom ones.
2. **Access level** — `dynamic` (entry-linked only), `tokens` (dynamic + custom), or `arbitrary` (all canvas properties). Pick `tokens` for most cases.
3. **Output path** — accept the default.
...
```
Generated and registered <N> components from Figma.

Source:     <component-path-from-add>
Tokens:     <register-design-tokens.ts>   (if design tokens were run)
Registered: via csdx studio:component:register

Open Studio → any composition → Components palette → look for the new tile.
Drop it on a canvas to verify the render matches the Figma source.

Re-run `csdx studio:component:sync --component-path=<path>` if you want this
new component (or any other code-side changes) to be visible to the next
Figma plugin run's Auto Map.
```

## Inputs needed from the user

1. `componentId` — required for the developer side (Steps 4–7). If absent, walk through Steps 0–3 first.
2. `designTokenId` — optional; only needed if the plugin produced tokens.
3. `projectId` — optional; only needed if the project isn't already linked.
4. `componentPathToSync` — optional; only needed if code components changed since last sync OR first run.
...
| Running `Register` on a path the CLI doesn't recognise as a component | Register fails — needs a component file (named export matching a React component) or a directory containing them | Pass the exact `.tsx` path returned by `Add`, or the parent directory if registering many at once |
...
- `/docs/20-bring-your-own-components/generate-components-from-figma.md` — the full developer-facing flow with reference diagram
- `/docs/20-bring-your-own-components/configure-design-tokens-in-studio.md`, `/docs/20-bring-your-own-components/import-design-tokens.md` — token references (source of truth for commands/flags)
