...
description: "Explain Section Slots — designated swap points inside a Section where a template author drops a different Section per instance without forking. Section-in-Slot, not raw component."
...
When a template uses this Section, the slot is empty until the author drags something into the outlined region on the canvas. Drop a `Button` → Studio renders `Button` inside the slot for that template. The next template can drop something else.
